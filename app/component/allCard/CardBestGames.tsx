...
const CardBestGames = () => {
const { bestOfTheYear, handleGetDetailGames } = useGame()
...
const dataBaru = bestOfTheYear.sort((a,b) => b.ratings_count - a.ratings_count)

return dataBaru.map((item:any, i:any) => {
  return (
    <div key={i} className='p-[7px]'>
    <div  className={`card-games w-[200px] h-[170px] lg:w-[380px] lg:h-[265px] pb-4 bg-slate-800 duration-300 rounded-xl ease-in-out ${values === i ? "scale-[1.03] rounded-b-none" : ""}`}onMouseEnter={() => handleHover(i)} onMouseLeave={() => handleHover(-1)}>
    <img className='w-full h-28 lg:h-48 mb-1 rounded-t-xl' src={item.background_image} alt='' />
    ...
    <div className='flex items-center'>
      <div className='flex gap-1 lg:gap-3'>
       ...
       <Image
       className='invert w-3 h-3 lg:w-[16px] lg:h-[16px]'
    ...
    <h1 className='lg:text-2xl line-clamp-1'>{item.name}</h1>
    </div>  
    </div>
    <div className={`detail absolute w-[200px] lg:w-[380px] text-slate-300 bg-slate-800 lg:text-xs text-[7px] px-3 py-1 rounded-b-lg ${values === i ? 'opacity-100 scale-[1.03] z-10' : "-z-10 hidden"}`} onMouseEnter={() => handleHover(i)} onMouseLeave={() => handleHover(-1)}>
        ...
        <Link onClick={() => handleGetDetailGames(item.id)} className='lg:text-[18px] text-[10px] border-b border-slate-300 hover:text-blue-500' href="/detail">Show More</Link>
